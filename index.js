...
console.log(createSymbolProperty("Це властивість об'єкту з використанням символу"));
		...
		return {
			next: () => {
				if (currentIndex < this.category.length) {
					const value = this.category[currentIndex].name;
					currentIndex++;
...
const useSymbolIterators = (object) => {
	const result = [];
